...
export default function BlendReportViewCpp3({ navigation }) {
  ...
  const getBlendDataDatewise = async (fromDate, toDate) => {
    try {
      const response = await axios.get(BaseUrl + "/blend/datewise/cpp3", {
        params: {
          fromdate: fromDate,
          todate: toDate,
        },
              ...
              >
                ...
                <Text style={{ fontSize: wp(5), fontWeight: "bold" }}>
                  {blendFeedingArray[index]
                    ? Number(blendFeedingArray[index].totalPathC || 0)
                    : "Loading..."}
                </Text>
              </View>
            </Card>
